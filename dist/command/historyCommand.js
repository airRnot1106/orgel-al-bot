...
class HistoryCommand extends absCommand_1.AbsCommand {
    ...
    async execute() {
        ...
        const historyList = ((await this._database.query(`SELECT title, author, requester_name FROM histories JOIN videos ON histories.video_id = videos.video_id JOIN requesters ON histories.requester_id = requesters.requester_id WHERE guild_id = '${guildId}' ORDER BY history_id DESC LIMIT ${limit};`)).rows);
